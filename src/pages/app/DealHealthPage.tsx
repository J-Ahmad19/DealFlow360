  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 card-tactile bg-white p-6 sm:p-10 min-h-[calc(100vh-4rem)] relative mb-12">
            ...
            {alerts.length === 0 ? (
              ...
              alerts.map((alert: any) => (
                <tr key={alert.id} className="hover:bg-slate-800/80 transition-colors">
                  <td className="px-6 py-5 font-black text-white">{alert.quotationId?.slice(0, 8)?.toUpperCase() || 'Unknown'}</td>
                  <td className="px-6 py-5 font-bold text-slate-200">
                    {alert.type === 'STALLED' ? 'Idle 9 days' : alert.type === 'DISCOUNT_ANOMALY' ? 'Discount 22% vs avg 8%' : alert.type === 'DELIVERY_SLIPPAGE' ? '3 promise dates at risk' : 'Approval delay'}
                  </td>
                  ...
                  <td className="px-6 py-5">
                    <span className={`inline-flex items-center rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-wider ${statusStyles[alert.severity?.toLowerCase() || 'medium']}`}>
                      {alert.severity === 'critical' ? 'Escalate' : alert.severity === 'high' ? 'Nudge rep' : 'Review'}
                    </span>
                  </td>
                </tr>
              ))
      ...
      <div className="flex flex-wrap items-center gap-3 pt-4">
        <button className="btn-tactile bg-red-400 border-2 border-red-500 text-white hover:bg-red-500 px-6 py-3 text-xs font-black rounded-xl">
          Escalate
        </button>
        <button className="btn-tactile bg-blue-500 border-2 border-blue-600 text-white hover:bg-blue-600 px-6 py-3 text-xs font-black rounded-xl">
          Nudge Rep
        </button>
      </div>
    </div>
